...
if __name__ == '__main__':
	...
	def on_log_notif(message_type, channel: int, transaction_id: int, error_code: int, payload: bytes):
		print(f"{time.time() - t0} - new log notif {len(payload)}")


	with AlcpBase(address) as ipg:
		ipg.connect()  # probar en q momento se tranca la descarga de datos, usando solo blep baje en 8 segundos tambien
		# blep._write(bytes.fromhex('03D50A2A0000FFFF0000000000001394C281'))
		# blep._write(bytes.fromhex('030A01F50000FFFF00000000000033EB67E2'))
		# blep._write(bytes.fromhex('030B010000000000900100F40000FFFF0000000000006B27BFCF'))

		ipg._subscribe_to_channel_notifications(0x0A, on_log_notif, send_order=False)
		ipg._send_alcp_order(0xD5, bytes.fromhex("0A"))
		input()
		ipg._send_alcp_order(0x0A, bytes.fromhex("01"))
		input()
		ipg._send_alcp_order(0x0B, bytes.fromhex("010000000000900100"))
		input()
