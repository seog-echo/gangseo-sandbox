...
def bigram_is_dubious(bigram):
    """Bit 14 of the 16-bit word is the dubious flag set by the device ADC."""
    return (bigram[0] >> 6) & 1
...
def chunk_to_samples(chunk_data):
    ...
    for i in range(0, usable_len, 4):
        ...
        samples.append(s1)
        samples.append(s2)
        dubious.append(bigram_is_dubious(chunk_data[i + 2 : i + 4]))
        dubious.append(bigram_is_dubious(chunk_data[i : i + 2]))
    return samples, dubious
...
class RecordingDecoder:
    ...
    def process_chunk(self, address, chunk_data, pad_samples=0, pad_front=False):
        """ 
            ...
            Ch3:              C           F
        """
        samples, dubious = chunk_to_samples(chunk_data)
        if pad_samples > 0:
            if pad_front:
                samples = ([0] * pad_samples) + samples
                dubious = ([0] * pad_samples) + dubious
            else:
                samples.extend([0] * pad_samples)
                dubious.extend([0] * pad_samples)
        ...
        dubious_lists = [[] for _ in range(self.num_channels)]
        for i, s in enumerate(samples):
            ch_idx = (start_channel_idx + i) % self.num_channels
            channel_lists[ch_idx].append(s)
            dubious_lists[ch_idx].append(dubious[i])

        return channel_lists, dubious_lists
