...
def resolve_ipg_address(device_number: int, config: Dict[str, Any]) -> str:
    ...
    address_path = repo_root / "address.py"
    if address_path.exists():
        text = address_path.read_text(encoding="utf-8")
        pattern = re.compile(
            r"^\s*#?\s*address\s*=\s*'([^']+)'\s*#\s*(?:IPG|BB)\s*(\d+)\s*$",
            re.MULTILINE,
        )
        for address, label_number in pattern.findall(text):
            if int(label_number) == int(device_number):
                return address

    modules = load_ipg_modules(repo_root)
    device_name = f"SIPGBB {int(device_number):02d}"
    address = modules["get_device_address_by_name"](device_name, timeout=config["ipg"]["connection"]["scan_timeout_s"], ensure_loop=True)
    if not address:
        raise RuntimeError(f"Unable to resolve IPG address for device {device_number}")
    return address
    ...
    def _recording_loop(self, recording_mask_hex: str, duration_seconds: int) -> None:
        ...
        def on_chunk(**kwargs: Any) -> None:
            ...
            try:
                ...
                channel_lists = decoder.process_chunk(header.BufferAddress, chunk_data)
                chunk_count += 1
                if channel_lists:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.ni_9202_indicator = QtWidgets.QLabel("NI-9202")
        ...
        ni_indicator_row.addWidget(self.ni_9202_indicator)
    ...
    def _on_ni_status(self, status: Dict[str, Any]) -> None:
        self.ni_connected = bool(status.get("NI-9202")) and bool(status.get("NI-9263"))

        ai_name = status.get("ai_device_name", "")
        ao_name = status.get("ao_device_name", "")
        self.ni_9202_indicator.setText(f"NI-9202 ({ai_name})" if ai_name else "NI-9202")
        self.ni_9263_indicator.setText(f"NI-9263 ({ao_name})" if ao_name else "NI-9263")
        self._set_indicator(self.ni_9202_indicator, bool(status.get("NI-9202")))
